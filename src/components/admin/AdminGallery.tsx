...
const AdminGallery = ({ handleGalleryTab }: any | boolean) => {
    return (
        <div className={`${handleGalleryTab ? '' : 'hidden'} flex flex-col justify-center items-center w-full`}>
                    ...
                    galleryData.map((gallery: IGalleryCard) => (
                        <div key={gallery.id} className=' w-full flex flex-row gap-5'>
                            <picture className='w-full h-full'>
                                <Image className='w-full h-full' src={gallery.image} alt={`${gallery.title} main photo`} />
                            </picture>
                            <div className='flex flex-col justify-around items-start'>
                                ...
                                <div className='flex flex-row gap-2'>
                                    ...
                                    <select>
                                        {
                                            galleryData.map((data: IGalleryCard) => <option key={data.id}>{data.title}</option>)
                                        }
                                    </select>
                                </div>
                                <div>
