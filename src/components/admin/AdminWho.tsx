...
const AdminWho = ({ handleWhoTab }: any | boolean) => {
    return (
        <div className={`${handleWhoTab ? '' : 'hidden'} flex flex-col justify-center items-center w-full`}>
            <form className='mt-10 flex flex-col gap-8 justify-center items-center w-full'>
                <div className='flex flex-col gap-3 w-3/4 justify-center items-start'>
                    ...
                    <textarea rows={18} maxLength={1400} className='bg-[--bg-secondary-color] rounded-lg p-2 w-full' id='who-text' />
                </div>
                <div className='flex flex-col gap-3 w-3/4 justify-center items-start'>
                    <label htmlFor='who-text'>Selecione a imagem da página 'Quem somos': (formato png, jpg, jpeg)</label>
