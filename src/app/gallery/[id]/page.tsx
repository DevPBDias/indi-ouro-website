...
const GalleryDetails = ({ params }: { params: { id: string | number } }) => {
    ...
    const filteredGallery = galleryData?.filter((gallery: IGalleryCard) => gallery.id === Number(galleryId))

    return (
        <main>
            <Suspense fallback={<Loading />}>
                <section className='flex flex-col flex-wrap px-[10%] gap-6 my-8'>
                    <h1 className='text-4xl uppercase font-bold text-[--primary-color]'>{filteredGallery[0]?.title}</h1>
